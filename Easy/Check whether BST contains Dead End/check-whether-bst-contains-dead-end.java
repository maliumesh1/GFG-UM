...
class Solution
{
    ...
    public static boolean dead(Node root,int left , int right)
    {
        if(root==null) return false;
        boolean l=false,r=false;
        if(root.left!=null) l=dead(root.left,left,root.data);
        if(root.right!=null) r=dead(root.right,root.data,right);
